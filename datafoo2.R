...
datafoo2 <- function(df, n.start = 6, n.stop = dim(df)[2], efekty = NULL, max.percentage = 0.1)
{
  df2 <- df
  ...
  # tryCatch({
    for(n in ind)
    {
      ...
      ######################################################
      if(length(pull(dd[, 2])) != length(0:max(df2$dni_specjalne)))
      {
        a <- pull(dd[, 1])
        b <- 0:max(df2$dni_specjalne)
        b <- b[!b %in% a]
        tib2 <- tibble("ds" = b, "val" = rep(0, length(b)))
        colnames(tib2) <- c("dni_specjalne", names(dd)[2])
        
        dd <- as_tibble(merge(tib2, dd, all = T))
      }
      #####################################################
      
      #średnie dla dni specjalnych
      means <- pull(dd[, 2])
      
      
      #zera wypełniane średnią z dnia specjalnego 
      zeros <- df$dni_specjalne[index]
      df2[ind2, n] <- NA
      df2[index, n] <- means[zeros+1]
      ...
      for (i in (ds + 1)) 
      {
        ...
        #uzupełnienie odchyleń średnimi
        df2[na.inx, n] <- means[df2[na.inx, 6]+1]
        
      }
    }
  # }, error = function(e){message(paste0("ERROR :",print(n), "\n"))})
  
  return(list(df.means = df2, df.nas = df3))
}
...
# data2[win, c(1:6, 80)]
dataset <- datafoo2(data, efekty = efekt.d1.d10, max.percentage = max.percentage)
data2 <- dataset$df.means
data3 <- dataset$df.nas
